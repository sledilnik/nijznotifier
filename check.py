...
def parse_page():
    resp = requests.get('https://www.nijz.si/sl/dnevno-spremljanje-okuzb-s-sars-cov-2-covid-19', headers=headers)
    resp.raise_for_status()
    tree = html.fromstring(resp.content)
    date = parser.parse(tree.xpath("//div[@class='meta']/text()")[0])
    links = tree.xpath("//div[contains(@class, 'field-name-body')]//a[contains(@href, '/sites/www.nijz.si/files/uploaded/')]/@href")
